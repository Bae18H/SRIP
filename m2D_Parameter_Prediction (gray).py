...
class Model(object):
    ...
    def __init__(self, model_path, train_mode=True, output_dim=27,  # Full model has 27 parameters
        ...
        with tf.name_scope('hidden'):
            ...
            h3 = tcl.flatten(h2)
            h3 = tcl.fully_connected(inputs=h3, num_outputs=self.output_dim, activation_fn=None)
                
        # Final (fully connected) layer; linearized wih sigmoid function
        with tf.name_scope('fully_connected'):
            self.logits = tcl.fully_connected(inputs=h3, num_outputs=self.output_dim, activation_fn=None)
        ...
        with tf.name_scope('optimize'):
            ...
            self.d_optimizer = tf.train.GradientDescentOptimizer(d_learning_rate)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(maxEpoch):
                ...
                _, Ld, param_pred = sess.run([self.d_optimizer, self.d_loss, self.ppred], 
                                             feed_dict={self.x: xtrain.eval(), self.y: ytrain.eval()})
                i+=1
                if i % 10 == 0:
                    ...
                    Ldv, param_v_pred = sess.run([self.d_loss, self.ppred],
                                                 feed_dict={self.x: xvalid.eval(), self.y: yvalid.eval()})
                    Ldvs.append(Ldv)
...
if __name__ == "__main__":
    ...
    with tf.name_scope('input'):
        ...
        filenames= [('C:/Users/Daniel/Desktop/Summer_Research/myc_e2f_data/figureG/gray_myc_ef_2_%d.png' %i) for i in range(1,501)]
        ...
        mat_data = io.loadmat('C:/Users/Daniel/Desktop/Summer_Research/myc_e2f_data/data_myc_ef_yao_2')
    ...
    with tf.Session() as sess:
        ...
        # Create datasets
        train_set = presult[:train_size, par_dim-2:par_dim], result[:train_size, 0:in_width, 0:in_height]
        valid_set = presult[-valid_size:, par_dim-2:par_dim], result[-valid_size:, 0:in_width, 0:in_height]     
