...
class Model(object):
    ...
    def data_loader(self, dataset, batchsize, shuffle=False): 
        with tf.Session() as sess:    
            ...
            pdb.set_trace()
            features, labels, peaks, pattern = sess.run(dataset)
            labels = np.repeat(labels, in_height/3, axis=2)
    ...
    def save_model(self, saver, sess, step):
        ...
        my_path = os.path.join(self.model_path, "mymodel.ckpt")
        saver.save(sess, os.path.join(os.getcwd(), my_path), global_step=step)
                

if __name__ == "__main__":
    # Load data from png file
    result = tf.image.decode_png(tf.read_file('C:/Users/Daniel/Desktop/Python/E2F vs kMYC Distribution.png'), channels=1)
    ...
    with tf.Session() as sess:
        ...
        train_set = result[:train_size, in_height:in_height+2], result[:train_size, 0:in_width], result[:train_size, -2:-1], result[:train_size, -1:] #parameters,distribution,peak value, pattern
        #print(sess.run(train_set)[0].shape, sess.run(train_set)[1].shape, sess.run(train_set)[2].shape, sess.run(train_set)[3].shape)
        #pdb.set_trace()
        valid_set = result[-valid_size:, in_height:in_height+2], result[-valid_size:, 0:in_width], result[-valid_size:,-2:-1], result[-valid_size:,-1:]
        train_mode = True
