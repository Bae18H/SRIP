...
def conv1d(x, x_filters, n_filers,
        ...
        # Creates random weights for each variable
...
class Model(object):
        ...
        def __init__(self, model_path, train_mode=True, output_dim=19, T=150,
                ...
                h0 = conv1d(y, 1, self.n_df, k_w=5, stride_w=2, padding="SAME", name="conv0")
                h0 = tf.nn.relu(tcl.batch_norm(h0))
                h1 = conv1d(h0, self.n_df, 2*self.n_df, k_w=5, stride_w=2, padding="SAME", name="conv1")
                h1 = tf.nn.relu(tcl.batch_norm(h1))
                h2 = conv1d(h1, 2*self.n_df, 4*self.n_df, k_w=5, stride_w=2, padding="SAME", name="conv2")
...
if __name__ == "__main__":
    ...
    with open('LeeMYC-E2FData.csv') as csvfile:
